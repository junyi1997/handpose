...
def main(argv):
    ...
    #                                  model_path_suffix,
    #                                  'test')
    model_save_dir = "models/checkpoint/{:}".format(FLAGS.pretrained_model)
    train_log_save_dir ="models/logs/checkpoint/train/"  
    test_log_save_dir ="models/logs/checkpoint/test/"  
    ...
    with tf.Session(config=tf.ConfigProto(device_count=device_count,
        ...
        if FLAGS.pretrained_model != '':
            if FLAGS.pretrained_model.endswith('.pkl'):
                ...
                #saver.restore(sess, tf.train.latest_checkpoint('/models/weights/checkpoint/'))
                saver.restore(sess, 'D:\datafingpose\model\weight\checkpoint')
                # check weights
                for variable in tf.trainable_variables():
                    with tf.variable_scope('', reuse=True):
                        var = tf.get_variable(variable.name.split(':0')[0])
